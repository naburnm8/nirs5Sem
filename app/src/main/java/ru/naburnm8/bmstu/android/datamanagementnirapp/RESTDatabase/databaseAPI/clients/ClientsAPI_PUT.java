...
public class ClientsAPI_PUT extends AsyncTask<Void, Void, ClientsResponse> {
    ...
    public ClientsAPI_PUT(RESTDBOutput context, String baseUrl, Clients object, String token) {
    ...
    protected ClientsResponse doInBackground(Void... voids) {
        try{
            Call<Clients> call = apiService.updateClient("Bearer " + token, object.getId(), object);
            ...
            lastLog = e.toString();
            Log.println(Log.ERROR, "ClientsAPI_PUT", e.toString());
        }
        return null;
    }
